...
class Soliloquy_Shortcode {
    ...
    public function shortcode( $atts ) {
        ...
        // Add in the custom slider styles.
        $slider .= $this->get_stylesheets( $data );
        ...
        $no_js   = '<noscript>';
        $no_js  .= apply_filters( 'soliloquy_output_no_js', '<style type="text/css">#soliloquy-container-' . sanitize_html_class( $data['id'] ) . '{opacity:1}#soliloquy-container-' . sanitize_html_class( $data['id'] ) . ' li > .soliloquy-caption{display:none}#soliloquy-container-' . sanitize_html_class( $data['id'] ) . ' li:first-child > .soliloquy-caption{display:block}</style>', $data );
    ...
    public function get_video_slide( $id, $item, $data, $i ) {
        ...
        $output .= '<a href="#" class="soliloquy-video-link" title="' . esc_attr( $item['title'] ) . '"' . apply_filters( 'soliloquy_output_link_attr', '', $id, $item, $data, $i ) . '>';
            ...
            if ( 1 === $i && ! $this->is_mobile() ) {
                $output .= '<img id="soliloquy-video-' . sanitize_html_class( $id ) . '" class="soliloquy-image soliloquy-video soliloquy-video-' . $vid_type . ' soliloquy-video-' . $i . '" src="' . esc_url( $imagesrc ) . '" alt="' . esc_attr( $item['title'] ) . '"' . apply_filters( 'soliloquy_output_image_attr', '', $id, $item, $data, $i ) . ' />';
            } else {
                $output .= '<img id="soliloquy-video-' . sanitize_html_class( $id ) . '" class="soliloquy-image soliloquy-video-thumb soliloquy-preload soliloquy-video-' . $vid_type . ' soliloquy-video-' . $i . '" src="' . esc_url( plugins_url( 'assets/css/images/holder.gif', dirname( dirname( __FILE__ ) ) ) ) . '" data-soliloquy-src="' . esc_url( $imagesrc ) . '" alt="' . esc_attr( $item['title'] ) . '"' . apply_filters( 'soliloquy_output_image_attr', '', $id, $item, $data, $i ) . ' />';
            }
    ...
    public function get_stylesheets( $data ) {
        ...
        return apply_filters( 'soliloquy_stylesheets', implode( '', $this->stylesheets ), $data );
    ...
    public function load_slider_theme( $theme ) {
        ...
        foreach ( Soliloquy_Common::get_instance()->get_slider_themes() as $array => $data ) {
            ...
            $this->stylesheets[] = '<link id="' . $this->base->plugin_slug . $theme . '-theme-style-css" rel="stylesheet" type="text/css" href="' . add_query_arg( 'ver', $this->base->version, plugins_url( 'themes/' . $theme . '/style.css', $data['file'] ) ) . '" media="all" />';
            break;
        }
    ...
    public function get_video_thumbnail( $id, $item, $data ) {
        ...
        if ( $thumb ) {
            ...
            if ( empty( $stream['error'] ) || isset( $stream['error'] ) && ! $stream['error'] ) {
                ...
                $data['slider'][$item['id']]['thumb'] = $thumb;
